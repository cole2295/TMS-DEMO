...
function CustomizeFlow() {
    if ($("#hdFunPackingStation").val() != "True") {
        $("#thStation").hide();
        $("#tdStation").hide();
    }
    if ($("#hdFunPackingDeliverCenter").val() != "True") {
...
function deleteBill() {
    ...
    var formCodes = getFormCodes(true);
...
function scanFormCode() {
    ...
    var boxNo = $("#lblBoxNo").html();
    var params = {};
    var weight = getWeight(true);
    //    if (weight == 0) {
    //        return;
    //    }
    posting = true;
    params.hidData = $("#hdInboundInitData").val();
    params.codeType = formType;
...
function printBox() {
    ...
    var formCodes = getFormCodes(false);
    if (posting) {
        alert('您的操作太快了，页面正在提交');
        return;
    }
...
function showPrintInfo() {
    ...
    openWindow('ShowPackingPrintInfoV2?boxNo=' + boxNo, 'PackingPrint', 800, 600);
}

function getFormCodes(onlySelect) {
    var formCodes = "";

    if (onlySelect) {
        $("#tblBillInfo tbody tr :checkbox:checked").each(function () {
            formCodes += $.trim($(this).parent().parent().find("td:eq(2)").html()) + ",";
        });
    } else {
...
function selectBoxToStation(box) {
    ...
    } else if (box.ArrivalCompanyFlag == "1") {
        //分拣中心
    ...
    else if (box.ArrivalCompanyFlag == "3") {
        //配送商
