...
namespace Vancl.TMS.Model.Common
{
    ...
    public static partial class ModelEx
    {
        ...
        public static void Trim<T>(this T searchModel) where T : BaseSearchModel, new()
        {
            Type type = typeof(T);
            PropertyInfo[] arrProperty = type.GetProperties();
            Type pitype = null;
        ...
        /// <returns></returns>
        public static string SequenceNextValue(this ISequenceable seq)
        {
            return seq.SequenceName + ".NEXTVAL";
        }

        public static String KeyCodeNextValue(this IKeyCodeable keycode)
        {
            if (String.IsNullOrWhiteSpace(keycode.SequenceName)) throw new ArgumentNullException("SequenceName is null");
            if (String.IsNullOrWhiteSpace(keycode.TableCode)) throw new ArgumentNullException("TableCode is null or empty");
            if (keycode.TableCode.Length != 3) throw new ArgumentNullException("TableCode's length <> 3");
            //TODO:当前默认为主库ST0，后续拆库后调用具体代码实现
            String DBCode = "ST0";

            return String.Format(@"'{0}' || '{1}'|| lpad({2}.NEXTVAL,14,'0')"
                , DBCode
                , keycode.TableCode
                , keycode.SequenceName);
